...
public class LogInApplication {
    ...
    // 판매자 로그인 토큰
    public String sellerLogInToken(LogInForm form) {

        // 로그인 구현 진행 순서
        // 1. 로그인 가능 여부 확인
        SellerEntity seller = sellerService.findValidSeller(form.getEmail(), form.getUserPassword())
                .orElseThrow(() -> new CustomException(CustomErrorCode.LOGIN_FAIL));

        // 2. 토큰 발행
        // 3. 토큰을 response 한다.
        return jwtAuthenticationTokenProvider.createToken(seller.getEmail(), seller.getId(), UserType.SELLER);
        // userPk - email, id - id, userType - UserType.CUSTOMER
